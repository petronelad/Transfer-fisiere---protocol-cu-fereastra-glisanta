...
class GUI():
    def __init__(self):
        #creare fereastra
        fereastra = tk.Tk()
        fereastra.title('Transfer de fisiere')

        #dimensiuni fereastra
        canvas = tk.Canvas(fereastra, height=500, width=1000)
        canvas.pack()

        #imaginea de fundal
        background_image = tk.PhotoImage(file='img.png')
        background_label = tk.Label(fereastra, image=background_image)
        background_label.place(relwidth=1, relheight=1)

        #casuta pentru Sender
        SenderView = tk.Frame(fereastra, bg='#000000', bd=5)
        ...
        ReceiverView = tk.Frame(fereastra, bg='#000000', bd=5)
        ...
        #buton Connect
        buttonC = tk.Button(text="Connect", bg='#00FF00')
        ...
        browse = tk.StringVar()
        browse_btn = tk.Button(fereastra, textvariable=browse, command=lambda: open_file(), font='Arial', bg='#ff9933',
        ...
        def open_file():
            ...
            fisier = askopenfile(parent=fereastra, mode='rb', title='Alege un fisier', filetype=[('Fisier text', '*.txt')])
            browse.set("Incarcat")
