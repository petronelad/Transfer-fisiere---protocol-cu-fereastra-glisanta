...
def receive_packet_and_send_ack(file_received, socket_received):
    ...
    while True:
        ...
        if id_packet_in_buffer == packet.id_packet:
            ...
            print("lost packet")
            print(id_packet_in_buffer.to_bytes(4, byteorder = 'little', signed = True) + packetS.State.UNKNOWN.value.to_bytes(4, byteorder = 'little', signed = True))
            socket_received.sendto(id_packet_in_buffer.to_bytes(4, byteorder = 'little', signed = True) + packetS.State.UNKNOWN.value.to_bytes(4, byteorder = 'little', signed = True),  source_address)
    file.close()
